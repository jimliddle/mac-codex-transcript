...
Codex Transcript turns the raw JSONL files under `~/.codex` into a browsable conversation view. It groups threads by project while keeping the project name prominent on every row, distinguishes user and model messages visually, wraps long content, and keeps technical activity out of the way until it is needed.
